...
namespace Business.Abstract.Base
{
    public class BaseBll<T, TContext> : IBaseBll where T : BaseEntity where TContext : DbContext
    {
        ...
        private bool Validation(IslemTuru islemTuru, BaseEntity oldEntity, BaseEntity currentEntity, Expression<Func<T, bool>> filter)
        {
            ...
            string GetValidationErrorControl()  // hatalı giriş varsa hangi kontroldeyse onu getirmek 
            {
                string MukerrerKod()
                {
                    foreach (var property in typeof(T).GetPropertyAttributesFromType<Kod>())
                    {
                        ...
                        if ((islemTuru == IslemTuru.EntityInsert || oldEntity.Kod == currentEntity.Kod) && islemTuru == IslemTuru.EntityUpdate) continue;// yeni bir kayıt ise yada birbirine eşitse true döndür true ise işlem yapma

                        if (_uow.Rep.Count(filter) < 1) continue;// aynı verileri engeller
                ...
                string HataliGiris()
                {
                    foreach (var property in typeof(T).GetPropertyAttributesFromType<ZorunluAlan>())
                    {
                        ...
                        Messages.HataliVeriMesai(property.Attribute.Description);
                        return property.Attribute.ControlName;
