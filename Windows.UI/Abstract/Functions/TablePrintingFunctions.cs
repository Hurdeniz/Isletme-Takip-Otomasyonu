using DevExpress.XtraGrid.Views.Grid;
using DevExpress.XtraPrinting;
using Entitiess.Concrete;
using Model.Enums;
using Model.Show;
using System;
using System.Collections.Generic;
using System.Drawing;
using System.Drawing.Printing;
using System.Linq;
using System.Text;
using System.Threading.Tasks;
using Windows.UI.Abstract.Show;
using Windows.UI.Concrete.GenelForms;

namespace Windows.UI.Abstract.Functions
{
    public class TablePrintingFunctions
    {
        private static GridView _tablo;
        private static PrintableComponentLink _link; //Başlıkları eklemek için kullanılacak class
        private static PrintingSystem _ps;//verilerin görüneceği bölüm
        private static DokumParametreleri _dp;


        public static void Yazdir(GridView tablo, string raporBaslik)
        {
            _link = new PrintableComponentLink();
            _ps = new PrintingSystem();
            _tablo = tablo;
            _dp = ShowEditForm<TabloYazdirForm>.ShowDialogEditForm<DokumParametreleri>(raporBaslik);

            RaporDokumu();
        }

        private static void RaporDokumu()
        {
            BaslikEkle();
            RaporuKagidaSigdir();

            _tablo.OptionsPrint.PrintHorzLines = _dp.YatayCizgileriGoster == EvetHayir.Evet; // yatay çizgiler
            _tablo.OptionsPrint.PrintHorzLines = _dp.DikeyCizgileriGoster == EvetHayir.Evet; //dikey çizgiler
            _tablo.OptionsPrint.PrintHeader = _dp.SutunBasliklariGoster == EvetHayir.Evet; // sütün başlıklar
            _tablo.OptionsView.ShowViewCaption = false;

            _link.Component = _tablo.GridControl;// tablodaki verileri dök
            _link.PaperKind = PaperKind.Letter; // kağıt boyutu
            _link.Margins = new Margins(59, 59, 115, 48);
            _link.CreateMarginalHeaderArea += Link_CreateMarginalHeaderArea;
            _link.CreateDocument(_ps);

            switch (_dp.DokumSekli)
            {
                case DokumSekli.TabloBaslikOnizleme:
                    ShowRibbonForms<RaporOnizleme>.ShowForm(true, _ps, _dp.RaporBaslik);
                    break;
                case DokumSekli.TabloYazdir:
                    for (int i = 0; i < _dp.YazdirilacakAdet; i++)
                        _link.Print(_dp.YaziciAdi); // seçilen yazıcıda kaç adet yazdırılacaksa gönder
                    break;
            }

            _tablo.OptionsView.ShowViewCaption = true;

        }

        private static void Link_CreateMarginalHeaderArea(object sender, CreateAreaEventArgs e)
        {
            if (_dp.BaslikEkle == EvetHayir.Hayir) return;

            var boldFont = new Font("Tahoma", 7, FontStyle.Bold);
            var regularFont = new Font("Tahoma", 7, FontStyle.Regular);

            // sayfa numara
            var sayfaBrick = new PageInfoBrick(BorderSide.None, 0, Color.Transparent, Color.Transparent, Color.Black)
            {
                Font = regularFont,
                PageInfo = PageInfo.NumberOfTotal,
                Format = "Sayfa : {0} / {1}",
                Alignment = BrickAlignment.Far,
                AutoWidth = true
            };

            _ps.Graph.DrawBrick(sayfaBrick, new RectangleF(200, 25, 40, 15)); // sanal dikdörtgen oluşturup içini doldurduk

            // tarih
            var tarihBrick = new PageInfoBrick(BorderSide.None, 0, Color.Transparent, Color.Transparent, Color.Black)
            {
                Font = regularFont,
                PageInfo = PageInfo.DateTime,
                Format = "Tarih : {0:dd.MM.yyyy}",
                Alignment = BrickAlignment.Far,
                AutoWidth = true
            };

            _ps.Graph.DrawBrick(tarihBrick, new RectangleF(0, 40, 50, 15));      
        }

        private static void RaporuKagidaSigdir()
        {
            YazdirmaYonuAyarla();

            switch (_dp.RaporKagidaSigdir)
            {
                case RaporKagidaSigdir.SutunlariDaraltarakSigdir:
                    _tablo.OptionsPrint.AutoWidth = true;
                    break;
                case RaporKagidaSigdir.YaziBoyutunuKuculterekSigdir:
                    _tablo.OptionsPrint.AutoWidth = false;
                    _ps.Document.AutoFitToPagesWidth = 1;
                    break;
                default:
                    _tablo.OptionsPrint.AutoWidth = false;
                    _ps.Document.ScaleFactor = 1.0f;
                    break;
            }


        }

        private static void YazdirmaYonuAyarla()
        {
            switch (_dp.YazdirmaYonu)
            {
                case YazdirmaYonu.Dikey:
                    _link.Landscape = false; //yatay yapma
                    break;
                case YazdirmaYonu.Yatay:
                    _link.Landscape = true; // yatay yap
                    break;
                case YazdirmaYonu.Otomatik:
                    _link.Landscape = OtomatikYazdirmaYonu();
                    break;
            }
        }

        private static bool OtomatikYazdirmaYonu()
        {
            const int sayfaGenisligi = 703;
            var tabloSutunGenislikleri = 0;

            for (int i = 0; i < _tablo.Columns.Count; i++)
                if (_tablo.Columns[i].Visible)
                    tabloSutunGenislikleri += _tablo.Columns[i].Width;

            return tabloSutunGenislikleri > sayfaGenisligi;
        }

        private static void BaslikEkle()
        {
            if (_dp.BaslikEkle == EvetHayir.Hayir) return;

            var headerArea = new PageHeaderArea(); //rapor üzerindeki kısım
            headerArea.Content.AddRange(new[] { "", _dp.RaporBaslik, "" });// orta başlık --sağ ve sol başlık boş
            headerArea.Font = new Font("Arial Narrow", 10f, FontStyle.Bold);// yazı stili
            headerArea.LineAlignment = BrickAlignment.Center;//yazı ortada
            _link.PageHeaderFooter = new PageHeaderFooter(headerArea, null);
        }
    }
}
